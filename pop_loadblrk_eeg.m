% pop_loadblrk_eeg() - load .nsX format dataset and
%                return only EEG Data in an EEGLAB EEG structure
%
% Usage:
%   >> [EEG, com] = pop_loadblrk_eeg; % pop-up window mode
%   >> [EEG, com] = pop_loadblrk_eeg(path, hdrfile);
%   >> [EEG, com] = pop_loadblrk_eeg(path, hdrfile, srange);
%   >> [EEG, com] = pop_loadblrk_eeg(path, hdrfile, [], chans);
%   >> [EEG, com] = pop_loadblrk_eeg(path, hdrfile, srange, chans);
%
% Optional inputs:
%   path      - path to files
...
function [EEG, com] = pop_loadblrk_eeg(path, hdrfile, srange, chans)
...
if nargin < 2
    ...
    uilist = {{ 'style' 'text' 'string' 'Interval (in seconds; e.g., [start stop]; default: entire signal'} ...
              ...
              { 'style' 'edit' 'string' ''}};
    result = inputgui(uigeom, uilist, 'pophelp(''pop_loadblrk_eeg'')', 'Load .nsX file');
...
% Header file
disp('pop_loadblrk_eeg(): opening file');
...
% Channel Infos
if ~exist('chans', 'var')
    chans = NS3.MetaTags.ChannelID(find(NS3.MetaTags.ChannelID<129));    
...
if isempty(chans)
    errordlg2('No EEG Electrodes associated with the data file. Hence Aborting.','Not an EEG Dataset');
    return
end
...
for chan = 1:length(chans)
    EEG.chanlocs(chan).labels = NS3.ElectrodesInfo(chans(chan)).Label;
end

disp('pop_loadblrk_eeg(): reading EEG data');
...
EEG.data = NS3.Data(chans,(srange(1)*EEG.srate+1):srange(2)*EEG.srate);
EEG.trials = 1;
EEG.xmin   = 0;
...
if nargout == 2
    com = sprintf('EEG = pop_loadblrk_eeg(''%s'', ''%s'', %s, %s);', path, hdrfile, mat2str(srange), mat2str(chans));
end
